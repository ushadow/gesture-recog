...
% 'term' - termination probability P(END | Q(T)) [ones(Q, 1), i.e. equal probability] 
%
% OUTPUTS:
...
[obslik2, mixmat, fwd_only, scaled, act, maximize, compute_xi, compute_gamma2, term] = ...
   process_options(varargin, ...
       'obslik2', [], 'mixmat', [], ...
       'fwd_only', 0, 'scaled', 1, 'act', [], 'maximize', 0, ...
       'compute_xi', compute_xi, 'compute_gamma2', compute_gamma2, 'term', ones(Q, 1));
...
if scaled
 if any(scale==0)
   ...
   loglik = sum(log(scale)) + log(alpha(:, T)' * term);
 end
else
 loglik = log(alpha(:,T)' * term);
end
