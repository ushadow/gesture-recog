function [loglik, errors, obslik] = mhmm_logprob(data, prior, transmat, ...
      mu, Sigma, mixmat, term, isDiag)
...
if nargin < 7, term = ones(Q, 1); end
if nargin < 8, isDiag = 0; end
...
errors = [];
for m = 1 : ncases
  obslik = mixgauss_prob(data{m}, mu, Sigma, mixmat, 0, isDiag);
  [~, ~, ~, ll] = fwdback(prior, transmat, obslik, 'fwd_only', 1, ...
    'term', term);
  if ll==-inf
    errors = [errors m];
  end
