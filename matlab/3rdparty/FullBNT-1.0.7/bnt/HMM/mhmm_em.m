function [LL, prior, transmat, mu, Sigma, mixmat, term] = ...
     mhmm_em(data, prior, transmat, mu, Sigma, mixmat, varargin)
% MHMM_EM Compute the ML parameters of an HMM with (mixtures of) Gaussians output using EM.
...
%
% End state related parameters:
% 'term' - termination probability [ones(Q, 1)]
% 'adj_term' - if 0, do not change term [1]
%
...
[max_iter, thresh, verbose, cov_type,  adj_prior, adj_trans, adj_mix, ...
    adj_mu, adj_Sigma, term, adj_term] = ...
...
while (num_iter <= max_iter) && ~converged
  % E step
  [loglik, exp_num_trans, exp_num_visits1, postmix, m, ip, op, exp_num_visitsT] = ...
      ess_mhmm(prior, transmat, mixmat, mu, Sigma, data, term);
...
function [loglik, exp_num_trans, exp_num_visits1, postmix, m, ip, op, exp_num_visitsT] = ...
    ess_mhmm(prior, transmat, mixmat, mu, Sigma, data, term)
