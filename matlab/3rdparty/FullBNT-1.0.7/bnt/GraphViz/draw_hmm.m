...
[thresh, B, startprob, endprob, filename, nodePos] = ...
...
for i=1:Q
  ...
  if ~isempty(endprob) && ~approxeq(endprob(i), 0)
    stop = '-';
  else
    stop = '';
  end
...
graph_to_dot(G, 'filename', filename, 'arc_label', arclabel, ...
            'node_label', nodelabel, 'nodePos', nodePos);
