function [h, spefound, senfound, espefound, esenfound] = roc(Output, Target, senval, speval, sim_name, silent, h, col, show_sen_spe)
%function [h, spefound, senfound, espefound, esenfound] = roc(Output, Target, senval, speval, sim_name, silent, h, col, show_sen_spe)
...
if nargin<9, show_sen_spe=1; end
...
if ~silent
    ...
    set(gca, 'YTick', [0:.1:1]);
    hold on
    if show_sen_spe, plot([0 1], [1 0], 'k'); end
    if ~overlay
    ...
    if ~isempty(senval) & show_sen_spe
        hold on
        plot(spefound, senval, 'ro');
    end
    if ~isempty(speval) & show_sen_spe
        hold on
        plot(speval, senfound, 'ro');
    end
    if ~isempty(senval) & ~isempty(speval) & show_sen_spe
        hold on
