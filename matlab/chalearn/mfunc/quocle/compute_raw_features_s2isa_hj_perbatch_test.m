function [Xall, MM, indices, data_file_names] = compute_raw_features_s2isa_hj_perbatch_test(rdatag,datag,network, params, data_file_names, size_idx, trflag, MTY, timseg)
...
%     end
%         V=datag{i};
%     vifile=[rdatag.datapath rdatag.dataname '/' MTY '_' num2str(rdatag.current_index) '.avi'];
%     cuix=saved_annotation{rdatag.current_index};

    M = loadclip_3dm(rdatag, fovea.spatial_size, 0, 0); 
% %     M = loadclip_3dm(rdatag{i}, fovea.spatial_size, 0, 0); 
%             M=M(:,:,timseg(i,1):timseg(i,2));
% %     if ~trflag,        
% %         M=M(:,:,cuix(1):cuix(2));
