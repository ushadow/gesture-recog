function result = draw_rectangle3(frame, top, bottom, left, right, color);
...
result = normalize_range(frame);
% we do (left-1):(left+1) to have a thicker line.
bands = size(result, 3);
for band = 1:bands
    result(top:bottom, [(left-1):(left+1), (right-1):(right+1)], band) = color(band);
    result([(top-1):(top+1), (bottom-1):(bottom+1)], left:right, band) = color(band);
end