...
while (frame < number_of_frames)
  ...
  % now we need to continue to find where that score is best.
  ...
  while(1)
    ...
    current_scores(current_starts <= last_finish) = inf;
    [best_score, best_model] = min(current_scores);
    if (best_model ~= current_best_model)
      break;
    end
  ...
  gesture_starts(label_index)  = start_frames (current_best_model, current_finish);
  gesture_finishes (label_index) =  current_finish;
  last_finish = current_finish;
%  frame = current_finish;
end
