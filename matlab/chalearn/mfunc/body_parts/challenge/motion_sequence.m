function result = motion_sequence(arg1, arg2, arg3)
...
vertical_size = size(frames{1}, 1);
horizontal_size = size(frames{1}, 2);
band_size = size(frames{1}, 3);

for  (counter =  2:(number-1))
  current_result = zeros (vertical_size, horizontal_size);
  previous_frame = double(frames {counter -1});
  current_frame = double(frames {counter});
  next_frame = double(frames {counter +1});
  ...
  result{counter-1} = current_result;
end
